...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', finetune=False):
        """
        Args:
    ...
    def forward(self, x):
        ...
        if self.head_dist is not None:
            ...
            if self.training and not torch.jit.is_scripting():
                # dense prediction for cskd
                N, M, C = x_dense.shape
                x_dense = x_dense.reshape(-1, C)
                x_dist_dense = self.head_dist(x_dense).reshape(N, M, -1)
                return x, x_dist, x_dist_dense
            else:
                # during inference, return the average
                N, M, C = x_dense.shape
                x_dense = x_dense.reshape(-1, C)
                x_dist_dense = self.head_dist(x_dense).reshape(N, M, -1)
                x_dist_dense = x_dist_dense.mean(dim=1)
                return (x + x_dist + x_dist_dense) / 3.
        else:
            x = self.head(x)
        return x
...
def finetune_deit_tiny_distilled_patch16_224(pretrained=False, **kwargs):
    ...
    model_kwargs = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3, **kwargs)
    model = _create_vision_transformer(
        'deit_tiny_distilled_patch16_224', pretrained=pretrained,  distilled=True, finetune=True, **model_kwargs)
    return model
...
def finetune_deit_tiny_distilled_patch16_384(pretrained=False, **kwargs):
    ...
    model_kwargs = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3, **kwargs)
    model = _create_vision_transformer(
        'deit_tiny_distilled_patch16_384', pretrained=pretrained,  distilled=True, finetune=True, **model_kwargs)
    return model
...
def finetune_deit_small_distilled_patch16_224(pretrained=False, **kwargs):
    ...
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, **kwargs)
    model = _create_vision_transformer(
        'deit_small_distilled_patch16_224', pretrained=pretrained,  distilled=True, finetune=True, **model_kwargs)
    return model
...
def finetune_deit_small_distilled_patch16_384(pretrained=False, **kwargs):
    ...
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, **kwargs)
    model = _create_vision_transformer(
        'deit_small_distilled_patch16_384', pretrained=pretrained,  distilled=True, finetune=True, **model_kwargs)
    return model
...
def finetune_deit_base_distilled_patch16_224(pretrained=False, **kwargs):
    ...
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer(
        'deit_base_distilled_patch16_224', pretrained=pretrained,  distilled=True, finetune=True, **model_kwargs)
    return model
...
def finetune_deit_base_distilled_patch16_384(pretrained=False, **kwargs):
    ...
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer(
        'deit_base_distilled_patch16_384', pretrained=pretrained,  distilled=True, finetune=True, **model_kwargs)
    return model
